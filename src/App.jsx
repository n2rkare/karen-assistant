...
async function saveValidTokens(tokens) {
  await window.storage.set(VALID_TOKENS_KEY, JSON.stringify(tokens), true);
}
...
// ─── System prompt ──────────────────────────────────────────────────────────
const SYSTEM_PROMPT = `You are Kare-N — a sharp, no-fluff AI executive assistant built specifically for independent funeral directors. You help them track tasks, remember follow-ups, and stay organized — because the work demands full presence and ADD-friendly systems matter.

Your behavior:
- When the user mentions something to do, capture it immediately as a task.
- When it has a date/time or deadline, note it clearly.
- Be direct, candid, and brief. No flattery. No softening. No filler.
- You have access to their current task list (provided below). Update it with every relevant interaction.
...
Be a thinking partner. Surface what's overdue. Ask clarifying questions when a deadline is vague. That's the job.`;
    ...
    <div style={{
      ...
      <div>
        ...
        <div style={{ color: "#64748b", fontSize: "14px", fontWeight: 500 }}>
          Your AI ops assistant for funeral professionals
        </div>
      </div>
      <div style={{
        background: "rgba(255,255,255,0.04)", border: "1px solid rgba(239,68,68,0.3)",
        borderRadius: "16px", padding: "24px 32px", maxWidth: "320px",
      }}>
        <div style={{ fontSize: "24px", marginBottom: "12px" }}>🔒</div>
        <div style={{ color: "#f1f5f9", fontWeight: 700, fontSize: "15px", marginBottom: "8px" }}>
          Access Required
        </div>
        <div style={{ color: "#64748b", fontSize: "13px", lineHeight: 1.6 }}>
          Kare-N is a member benefit of The Practitioner community. Your access link should have been sent when you joined.
        </div>
      </div>
      <div style={{ color: "#334155", fontSize: "12px" }}>
        Already a member? Check your welcome email for your personal access link.
      </div>
    </div>
  );
}
...
function AdminPanel() {
  ...
  async function addMember() {
    ...
    const updated = { ...tokens, [token]: { name: newName.trim(), createdAt: new Date().toISOString(), active: true } };
    await saveValidTokens(updated);
    setTokens(updated);
    setNewName("");
  }
  ...
  const s = {
    page: {
      minHeight: "100vh", background: "linear-gradient(135deg, #0f172a 0%, #1a1035 50%, #0f172a 100%)",
      fontFamily: "'Nunito', sans-serif", color: "#e2e8f0", padding: "24px 20px",
    },
    card: {
    ...
    btn: (color = "#a78bfa") => ({
      ...
      border: `1px solid ${color}44`, borderRadius: "10px", color: "#fff",
    ...
    <div style={{ ...s.page, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ width: "100%", maxWidth: "360px" }}>
        <div style={{ textAlign: "center", marginBottom: "32px" }}>
          ...
          <div style={{
            fontSize: "24px", fontWeight: 800, marginTop: "12px",
            background: "linear-gradient(90deg, #22d3ee, #a78bfa)",
            WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text",
          }}>Kare-N Admin</div>
        </div>
        <div style={s.card}>
          ...
          <input
            ...
            style={{ ...s.input, marginBottom: "12px", borderColor: pwError ? "#ef4444" : "rgba(167,139,250,0.2)" }}
          />
          {pwError && <div style={{ color: "#ef4444", fontSize: "12px", marginBottom: "12px" }}>Incorrect password.</div>}
          <button onClick={handleLogin} style={{ ...s.btn(), width: "100%", padding: "11px" }}>
            Enter
          </button>
        </div>
      </div>
  ...
  const memberList = Object.entries(tokens);
      ...
      <div style={{ maxWidth: "600px", margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "24px" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: "11px", color: "#64748b" }}>{memberList.length} active member{memberList.length !== 1 ? "s" : ""}</div>
          </div>
        </div>
        ...
        <div style={s.card}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            <input
              ...
              placeholder="Member name (e.g. Jane Smith)"
              style={{ ...s.input, flex: 1 }}
            />
            <button onClick={addMember} style={s.btn("#22d3ee")}>
              + Add
            </button>
          </div>
        </div>
        ...
        {/* Member list */}
        <div style={s.card}>
          <label style={s.label}>Active Members</label>
          {loading ? (
            <div style={{ color: "#64748b", fontSize: "13px" }}>Loading...</div>
          ) : memberList.length === 0 ? (
            <div style={{ color: "#334155", fontSize: "13px" }}>No members yet. Add one above.</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {memberList.map(([token, member]) => (
                <div key={token} style={{
                  ...
                  <div style={{
                    ...
                    background: "linear-gradient(135deg, #22d3ee44, #a78bfa44)",
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: "14px", marginBottom: "3px" }}>{member.name}</div>
                  ...
                  <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
                    <button onClick={() => copyUrl(token)} style={{
                      ...s.btn("#22d3ee"),
                      padding: "6px 12px", fontSize: "11px",
                      background: copied === token ? "rgba(52,211,153,0.2)" : "rgba(34,211,238,0.1)",
                      borderColor: copied === token ? "#34d399" : "rgba(34,211,238,0.3)",
                    }}>
                      {copied === token ? "✓ Copied" : "📋 Copy URL"}
                    </button>
                    <button onClick={() => revokeToken(token)} style={{
                      ...s.btn("#ef4444"),
                      padding: "6px 12px", fontSize: "11px",
        ...
        <div style={{ fontSize: "11px", color: "#1e293b", textAlign: "center" }}>
          Kare-N · The Practitioner · Member Access System
        </div>
      </div>
    </div>
  );
...
function KarenMain({ token }) {
  ...
  const [messages, setMessages] = useState([
    { role: "assistant", content: "Hey. What's on your plate — or want me to pull up what's pending?" },
  ]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get(TASK_KEY);
  ...
  async function saveTasks(t) {
    setTasks(t);
    try { await window.storage.set(TASK_KEY, JSON.stringify(t)); } catch (_) {}
  }
...
"/api/chat",,
  ...
  async function deleteTask(id) { await saveTasks(tasks.filter(t => t.id !== id)); }
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .glow-teal { position:fixed;width:300px;height:300px;border-radius:50%;background:radial-gradient(circle,#22d3ee18 0%,transparent 70%);top:-100px;right:-100px;pointer-events:none;z-index:0; }
        .glow-purple { position:fixed;width:250px;height:250px;border-radius:50%;background:radial-gradient(circle,#a78bfa14 0%,transparent 70%);bottom:100px;left:-80px;pointer-events:none;z-index:0; }
      `}</style>
      <div className="glow-teal" /><div className="glow-purple" />
      ...
      <div style={{ padding:"16px 20px 12px", display:"flex", alignItems:"center", gap:"12px", background:"rgba(255,255,255,0.03)", borderBottom:"1px solid rgba(167,139,250,0.15)", backdropFilter:"blur(10px)", position:"relative", zIndex:10 }}>
        ...
        <div style={{ display:"flex", gap:"8px", fontSize:"11px", textAlign:"center" }}>
          {overdue.length > 0 && (
            <div style={{ background:"rgba(239,68,68,0.15)", border:"1px solid rgba(239,68,68,0.3)", borderRadius:"8px", padding:"4px 10px" }}>
              <div style={{ color:"#ef4444", fontWeight:700, fontSize:"14px" }}>{overdue.length}</div>
              <div style={{ color:"#ef4444", opacity:0.7 }}>overdue</div>
            </div>
          )}
          <div style={{ background:"rgba(167,139,250,0.1)", border:"1px solid rgba(167,139,250,0.2)", borderRadius:"8px", padding:"4px 10px" }}>
            <div style={{ color:"#a78bfa", fontWeight:700, fontSize:"14px" }}>{pending.length}</div>
            <div style={{ color:"#a78bfa", opacity:0.7 }}>pending</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"flex", background:"rgba(255,255,255,0.02)", borderBottom:"1px solid rgba(167,139,250,0.1)", position:"relative", zIndex:10 }}>
        {["chat","tasks"].map(tab => (
          <button key={tab} className="tab-btn" onClick={() => setActiveTab(tab)} style={{ flex:1, padding:"12px", background:"none", border:"none", borderBottom:`2px solid ${activeTab===tab?"#a78bfa":"transparent"}`, color:activeTab===tab?"#a78bfa":"#64748b", cursor:"pointer", fontFamily:"inherit", fontSize:"12px", fontWeight:700, letterSpacing:"1px", textTransform:"uppercase", transition:"all 0.15s" }}>
            {tab}
            {tab==="tasks" && tasks.length>0 && <span style={{ marginLeft:"6px", background:"rgba(167,139,250,0.2)", color:"#a78bfa", padding:"1px 7px", borderRadius:"10px", fontSize:"10px" }}>{tasks.length}</span>}
          </button>
        ))}
      </div>
      ...
      {activeTab === "chat" && (
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", position:"relative", zIndex:5 }}>
          <div style={{ flex:1, overflowY:"auto", padding:"16px", display:"flex", flexDirection:"column", gap:"14px" }}>
            {messages.map((m, i) => (
              <div key={i} className="msg-bubble" style={{ display:"flex", flexDirection:m.role==="user"?"row-reverse":"row", gap:"10px", alignItems:"flex-end" }}>
                ...
                <div style={{ maxWidth:"78%", padding:"12px 16px", borderRadius:m.role==="user"?"18px 18px 4px 18px":"4px 18px 18px 18px", background:m.role==="user"?"linear-gradient(135deg,#1e3a5f,#1a1045)":"rgba(255,255,255,0.05)", border:m.role==="user"?"1px solid rgba(34,211,238,0.2)":"1px solid rgba(167,139,250,0.15)", fontSize:"14px", lineHeight:"1.6", color:m.role==="user"?"#bae6fd":"#cbd5e1", whiteSpace:"pre-wrap", backdropFilter:"blur(4px)" }}>
                  {m.content}
                </div>
              </div>
            ))}
          ...
          <div style={{ padding:"12px 16px", borderTop:"1px solid rgba(167,139,250,0.1)", background:"rgba(15,23,42,0.8)", backdropFilter:"blur(10px)", display:"flex", gap:"10px", alignItems:"flex-end" }}>
            <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMessage();} }} placeholder="Tell me what you need to do..." rows={2}
              style={{ flex:1, background:"rgba(255,255,255,0.05)", border:"1px solid rgba(167,139,250,0.2)", borderRadius:"14px", color:"#e2e8f0", padding:"10px 14px", fontSize:"14px", fontFamily:"inherit", outline:"none", lineHeight:"1.5" }} />
          ...
          <div style={{ display:"flex", gap:"6px", flexWrap:"wrap", marginBottom:"16px" }}>
            {["all","pending","overdue","done",...categories].map(f => (
              <button key={f} className="filter-pill" onClick={() => setFilter(f)} style={{ padding:"5px 14px", background:filter===f?"linear-gradient(135deg,#22d3ee88,#a78bfa88)":"rgba(255,255,255,0.04)", color:filter===f?"#fff":"#64748b", border:`1px solid ${filter===f?"rgba(167,139,250,0.5)":"rgba(255,255,255,0.08)"}`, borderRadius:"20px", cursor:"pointer", fontSize:"11px", fontFamily:"inherit", fontWeight:600, letterSpacing:"0.5px", textTransform:"uppercase" }}>
                {f}
                {f==="overdue"&&overdue.length>0&&<span style={{ marginLeft:"5px", background:"#ef4444", color:"#fff", borderRadius:"10px", padding:"0 5px", fontSize:"9px" }}>{overdue.length}</span>}
              </button>
            ))}
          </div>
          {sorted.length === 0 ? (
            <div style={{ textAlign:"center", marginTop:"60px" }}>
              ...
              <div style={{ color:"#334155", fontSize:"13px", marginTop:"16px", fontWeight:500 }}>
                {tasks.length===0?"No tasks yet — tell me what you need to do.":"Nothing in this filter."}
              </div>
            </div>
          ) : (
            <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
